...
<template>
  <div class="student-box mt-[108px] flex justify-center lg:ml-[20%] lg:mt-[60px]">
    <div class="mb-[20px] flex h-full w-full flex-col mx-auto bg-se-dark p-[20px] lg:w-[80%] lg:flex-row">
      <!-- Info -->
          ...
          <div class="mx-auto w-[80%] border-t-2 border-se-gray-light pt-4 text-left">
            <form @submit.prevent="updateInfo">
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                ...
                <BaseInput v-else type="text" :placeholder="student.studentId" v-model="studentUpdated.studentId"
              ...
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                ...
                <BaseInput v-else type="text" :placeholder="student.firstname" v-model="studentUpdated.firstname"
                  class="h-auto rounded text-se-dark sm:w-1/2" />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                <span class="font-semibold sm:w-1/2">Last Name:</span>
                <span v-if="updating === false" class="sm:w-1/2">{{ student.surname }}</span>
                <BaseInput v-else type="text" :placeholder="student.surname" v-model="studentUpdated.surname"
                  class="h-auto rounded text-se-dark sm:w-1/2" />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                <span class="font-semibold sm:w-1/2">Department:</span>
                <span v-if="updating === false" class="sm:w-1/2">{{ student.department }}</span>
                <BaseInput v-else type="text" :placeholder="student.department" v-model="studentUpdated.department"
                  class="h-auto rounded text-se-dark sm:w-1/2" />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                <span class="font-semibold sm:w-1/2">Advisor:</span>
                <span v-if="updating === false && student.advisor !== null" class="sm:w-1/2">
                  <RouterLink v-if="updating === false && student.advisor !== null"
                    :to="{ name: 'advisorDetail', params: { id: student?.advisor.id } }"
                    class="transition hover:scale-[1.02] hover:text-se-color-light">
                    <span class="underline underline-offset-4">{{ student?.advisor.firstname }} {{
                      student?.advisor.surname }}</span>
                  </RouterLink>
                </span>
                <span v-else-if="updating === false && student.advisor === null" class="sm:w-1/2">
                  -
                </span>
                <BaseSelect v-else-if="updating === true" v-model="studentUpdated.advisor.id" :options="advisors"
                  :key-extractor="(x) => x.id" :value-extractor="(x) => x.id"
                  :text-extractor="(x) => x.firstname + ' ' + x.surname" label=""
