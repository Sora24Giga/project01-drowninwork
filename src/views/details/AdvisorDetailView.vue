...
<template>
  <div class="mt-[138px] flex justify-center lg:mt-[80px]">
    <div class="mb-[20px] flex h-full w-full justify-center bg-se-dark lg:ml-[17%]">
      <div class="flex w-[90%] min-w-[216px] max-w-[500px] flex-col justify-center lg:w-[60%]">
        <p
        ...
        >
          <span class="text-xl font-semibold"
            >{{ advisor.firstname.toLocaleUpperCase() }}
            ...
            <form @submit.prevent="updateInfo">
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                ...
                <BaseInput
                  ...
                  v-model="advisorUpdated.firstname"
                  class="h-auto mb-1 rounded text-se-dark sm:w-1/2"
                />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                ...
                <BaseInput
                  v-else
                  type="text"
                  :placeholder="advisor.surname"
                  v-model="advisorUpdated.surname"
                  class="h-auto mb-1 rounded text-se-dark sm:w-1/2"
                />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                <span class="font-semibold sm:w-1/2">Academic Position:</span>
                <span v-if="updating === false" class="sm:w-1/2">{{
                  advisor.academicPosition
                }}</span>
                <BaseInput
                  v-else
                  type="text"
                  :placeholder="advisor.academicPosition"
                  v-model="advisorUpdated.academicPosition"
                  class="h-auto mb-1 rounded text-se-dark sm:w-1/2"
                />
              </div>
              <div class="flex flex-col w-full mb-1 justify-evenly sm:flex-row">
                ...
                <BaseInput
                  v-else
                  type="text"
                  :placeholder="advisor.department"
                  v-model="advisorUpdated.department"
                  class="h-auto mb-1 rounded text-se-dark sm:w-1/2"
                />
              </div>
